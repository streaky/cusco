...
The implemented ownership model is **Approach A: native-owned opaque physical state**. The ABI does not expose unstable llama sequence IDs, raw allocator internals, or an assumption that one Rust logical chunk equals one physical allocation. It exposes versioned opaque representation handles, component coverage, authoritative size and placement, retain/release ownership, completion fences, and prepare/validate/commit/abort operations. This keeps model- and backend-specific layout knowledge inside llama.cpp while allowing Rust to own policy and the publication linearization point.
