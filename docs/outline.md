...
**Repository status (2026-08-08):** The implementation provides the deterministic `window_tail` strategy, bounded and expiring declaration lifecycle, bounded compaction-worker admission that cannot queue behind or consume ordinary interactive admission, request/response plumbing, correlation/session metadata, Responses lifecycle events, root `cusco.compaction_result` replay metadata on OpenAI terminal responses, model-aware textual terminal-sequence suppression, and prepare/evaluate/validate-before-publish successor semantics. The rebuilt Compose coverage gate passes (`97 passed` in `cusco-server`, all measured Rust files above the 80% threshold, including `context_strategy.rs` at 99.68% and `lib.rs` at 91.46%), and the real-GPU API smoke validates both the full `window_tail` request path and a subsequent request continuing from its published successor (`11/11` scenarios). Phase 10's window-tail baseline exit checklist is complete; external workers remain disabled pending the post-v1 isolation work.
