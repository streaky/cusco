...
One inference adapter and one bounded management compatibility profile are committed, while another inference protocol remains a post-completion option:

1. **OpenAI-compatible `/openai/v1`:** the sole public inference contract. The minimal server's existing `/v1` surface migrates to this namespace during the Phase 9 clean cutover.
...
The canonical model-lifecycle service belongs to the Cusco control plane. A bounded Ollama-compatible management profile exposes:

- `GET /cusco/v1/api/tags` to list installed models, immutable revisions, aliases, sizes, and load state;
- `POST /cusco/v1/api/show` to report source provenance and executor metadata;
- `POST /cusco/v1/api/pull` to install or converge a model to the requested source identity;
- `POST /cusco/v1/api/copy` to assign another public name without changing immutable model identity;
- `DELETE /cusco/v1/api/delete` to remove a model record and eligible artifact content, or report why loaded, pinned, or referenced state prevents removal;
...
Local files require no native registration exception: they are discovered exclusively from `user.yaml` and exposed through the same model list and show views as pulled artifacts. Separate native register, list, show, pull/fetch, update-check, verify, alias/copy, and delete routes should be removed once their compatibility-profile equivalents and startup reconciliation are complete.

Downloads, updates, verification, repair, and deletions require per-model coordination, temporary-file cleanup, atomic publication, capacity checks, and cancellation. Cache management must distinguish the Hugging Face artifact cache from the evaluated model-state cache described elsewhere in this proposal. The compatibility handlers and CLI call the same protocol-neutral `ModelService`; neither owns an independent model store.
...
- an Ollama-compatible model-management profile under `/cusco/v1/api/*`, without Ollama generation or chat, over the same protocol-neutral lifecycle services;
- an explicit versioned `/cusco/v1/*` control API for logical contexts, branches, cache and compaction policy, activity hints, extended usage, cancellation, and richer lifecycle controls;
- generated, checked OpenAPI descriptions for the `/openai/v1/*` and `/cusco/v1/*` surfaces;
- protocol-neutral request, streaming, error, usage, and multimodal content types informed by OpenAI and Anthropic semantics without promising an Anthropic wire adapter;
...
- Ollama compatibility is deliberately limited to model management under `/cusco/v1/api/*`: version, model list/show, convergent pull, copy/alias, delete, and residency reporting. OpenAI under `/openai/v1/*` is the sole inference API and owns generation, streaming, vision, tools, structured output, Responses lineage, and compaction request controls. Cusco-specific context, scheduling, cancellation, usage, and lifecycle capabilities remain available through `/cusco/v1/*`.
...
- Vision content uses an ordered canonical sequence of text and image parts. OpenAI-compatible adapters preserve that sequence directly. Clients may provide explicit image labels or filenames when association matters, and Cusco must not claim equivalence where an eventual adapter cannot represent it.
...
- Vision admission failures use stable machine-readable error codes with structured details where applicable, including the violated field, observed value, limit, unit, selected model/projector, and whether the failure is due to malformed input, unsupported capability, resource safety, context budget, or unavailable retained history. HTTP, CLI, and OpenAI adapters preserve the canonical error category while applying their wire-specific envelopes.
...
- Tool streaming uses one canonical incremental internal representation while the OpenAI adapter emits the compatible wire format. V1 may stream tool-call argument deltas, but a call is executable only after its arguments are complete and validated. External tool results are submitted atomically in v1; incremental result streaming is deferred unless a compatible protocol contract is defined later.
- Tool choice follows the conservative OpenAI-compatible contract: `auto`, `none`, and `required` are supported where the model can honor them, and forcing a named function is supported where an equivalent model capability exists. Cusco rejects unsupported or impossible choices explicitly rather than silently falling back to `auto`.
- Tool-bearing requests are limited to message-oriented endpoints with an unambiguous continuation model: OpenAI chat and Responses-style APIs support tools and tool results. Plain text completions reject tools and tool-result messages in v1 rather than inventing a prompt-only representation. Every accepted tool result must belong to an active tool-call continuation.
...
- Pull and inference remain separate operations at different service layers. Inference has priority over pull where resources conflict. The Ollama-compatible `/cusco/v1/api/pull` endpoint streams progress until completion, including terminal success or error, rather than introducing a separate job-status API. Pull may be backed by asynchronous server work, must handle client cancellation and disconnects, and must shut down politely without compromising active inference or immutable model publication.
...
- The API is a clean cutover rather than a compatibility migration: remove the unprefixed `/v1/*`, `/native/*`, and standalone `/ollama/*` routes entirely; expose inference under `/openai/v1/*` and the control plane, including its management compatibility profile, under `/cusco/v1/*`; and update all callers, tests, OpenAPI documents, examples, and operator documentation in the same change. No aliases, deprecation window, or backward-compatibility shim is required for this fresh single-user project.
...
- deliver the bounded Ollama-compatible model-management profile under `/cusco/v1/api/*` over canonical Cusco lifecycle services, including convergent pull with integrated update resolution, verification, and repair, without Ollama generation or chat;
...
- `window_tail`
  ...
  - Treats `target` as a target, not a hard cap: if policy anchors alone exceed the target and cannot be reduced, compaction returns an `AnchorsOnly` result (no additional conversational window), continues execution, and emits runtime metadata with the `AnchorsOnly` reason so clients can distinguish this from admission/resource failures.
...
**Repository status (2026-08-08):** The Phase 10 v1 baseline is complete. The implementation provides the deterministic `window_tail` strategy, bounded compaction-worker admission that cannot queue behind or consume ordinary interactive admission, request/response plumbing, correlation/session metadata, Responses lifecycle events, root `cusco.compaction_result` replay metadata on OpenAI terminal responses, model-aware textual terminal-sequence suppression, and prepare/evaluate/validate-before-publish successor semantics. Published logical contexts retain both the submitted turn and generated assistant output, so later compaction operates on the same conversation represented by native state. Declarations are principal-owned at creation and consumption, bounded by count, lifetime, and per-principal rate, and explicitly revocable with `DELETE /cusco/v1/compaction/declarations/{id}`. The semantic matrix is backed by repository fixtures and checks deterministic pronoun, instruction, tool-contract, and follow-up retention. The rebuilt Compose coverage gate passes (`97 passed` in `cusco-server`; every measured Rust file remains above 80%, including `context_strategy.rs` at 99.70% and `lib.rs` at 91.32%), and the real-GPU API smoke passes all `11/11` scenarios, including compaction followed by continuation from the published successor. External workers remain disabled pending the post-v1 isolation work.
...
| Operator controls | `compaction_operator_limits_bound_workers_declarations_and_rate`, `saturated_compaction_workers_do_not_block_interactive_inference` | Configuration can disable compaction, bounds declaration count/lifetime/rate and worker concurrency, rejects saturated compaction work without queueing, and leaves ordinary interactive inference available. |
| Documentation | `swagger_combined_spec_and_ui` | A single combined OpenAPI JSON is the source of truth for `/openai/v1/*` and `/cusco/v1/*`, including the Ollama-compatible management profile under `/cusco/v1/api/*`; it contains no standalone `/ollama/*` paths or Ollama inference operations. The Swagger UI route is opt-in and **disabled by default** through `config.yaml` (e.g., `openapi.ui.enabled: false`) and must require explicit operator enablement in all other environments. |
