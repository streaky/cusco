...
RUN apt-get update && apt-get install -y --no-install-recommends ccache curl ca-certificates cmake ninja-build python3 python3-pip clang llvm git build-essential pkg-config && rm -rf /var/lib/apt/lists/* && python3 -m pip install --break-system-packages --no-cache-dir openai==2.11.0 && curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --default-toolchain 1.85.0 && /root/.cargo/bin/cargo install cargo-llvm-cov --version 0.6.16 --locked
