...
RUN apt-get update && apt-get install -y --no-install-recommends ccache curl ca-certificates cmake ninja-build python3 python3-venv clang llvm git build-essential pkg-config && rm -rf /var/lib/apt/lists/* && python3 -m venv /opt/cusco-venv && /opt/cusco-venv/bin/pip install --no-cache-dir openai==2.11.0 && curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y --default-toolchain 1.85.0 && /root/.cargo/bin/cargo install cargo-llvm-cov --version 0.6.16 --locked
RUN /root/.cargo/bin/rustup component add llvm-tools-preview
ENV PATH=/opt/cusco-venv/bin:/root/.cargo/bin:$PATH
WORKDIR /work
...
COPY . .
RUN touch Cargo.toml crates/*/Cargo.toml crates/*/build.rs crates/*/src/*.rs crates/*/tests/*.rs native/include/*.h native/shim/*.cpp
